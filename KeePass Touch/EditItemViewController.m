...
- (id)init {
  ...
  if (self) {
    ...
    self.imageButtonCell = [[ImageButtonCell alloc]
        initWithLabel:NSLocalizedString(@"Image", nil)];
    [self.imageButtonCell.imageButton addTarget:self
                                         action:@selector(imageButtonPressed)
                               forControlEvents:UIControlEventTouchUpInside];

    self.controls = [NSArray
        arrayWithObjects:self.nameTextField, self.imageButtonCell, nil];
  }
  return self;
}
  ...
  if (self) {
    ...
    if (self.isKdb4) {
      ...
      if (grp4.customIconUuid != nil) {
        self.searchKey =
            [[NSString alloc] initWithData:grp4.customIconUuid.getData
                                  encoding:NSASCIIStringEncoding];
        [self setSelectedImageCustom];
      } else {
...
- (void)setSelectedImageIndex:(NSUInteger)selectedImageIndex {
  ...
  UIImage *image =
      [[ImageFactory sharedInstance] imageForIndex:selectedImageIndex];

  [self.imageButtonCell.imageButton setImage:image
