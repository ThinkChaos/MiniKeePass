...
- (id)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    UIMenuItem *username =
        [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"Username", nil)
                                   action:@selector(pasteUsername:)];
    UIMenuItem *password =
        [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"Password", nil)
                                   action:@selector(pastePassword:)];
...
- (void)viewDidLoad {
  ...
  NSArray *items =
      @[ [UIImage imageNamed:@"user"], [UIImage imageNamed:@"asterisk"] ];
  ...
  self.openInButton = [[UIBarButtonItem alloc]
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
  [UIView animateWithDuration:0.4
                   animations:^{
                     ...
                     [self.navigationItem
                         setRightBarButtonItem:self.autotypeButtons
                                      animated:YES];
...
- (void)updateButtons {
  ...
  UIBarButtonItem *reloadOrStopButton =
      self.webView.loading ? self.stopButton : self.reloadButton;

  self.toolbarItems = @[
    fixedSpace, self.backButton, flexibleSpace, self.forwardButton,
    flexibleSpace, reloadOrStopButton, flexibleSpace, self.openInButton,
    fixedSpace
  ];
}
