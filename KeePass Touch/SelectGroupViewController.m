...
- (void)layoutSubviews {
  ...
  if ([self respondsToSelector:@selector(setSeparatorInset:)]) {
    self.imageView.frame = CGRectMake(
        self.spacerWidth + (imageWidth + self.spacerWidth) * self.level,
    ...
    self.contentView.frame =
        CGRectMake(indentation, self.contentView.frame.origin.y,
                   self.contentView.frame.size.width - indentation,
                   self.contentView.frame.size.height);
    self.textLabel.frame =
        CGRectMake(self.textLabel.frame.origin.x, self.textLabel.frame.origin.y,
...
- (void)viewDidLoad {
  ...
  NSString *filename =
      [self.appDelegate.databaseDocument.filename lastPathComponent];
...
- (void)addGroup:(KdbGroup *)group
  ...
  BOOL selectable = [self.delegate selectGroupViewController:self
  ...
  NSArray *sortedGroups = [group.groups
      sortedArrayUsingComparator:^NSComparisonResult(KdbGroup *group1,
                                                     KdbGroup *group2) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  IndentTableViewCell *cell =
      [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
  if (cell == nil) {
    cell =
        [[IndentTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
