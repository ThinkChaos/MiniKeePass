...
@interface FTPSelectViewController () <UITableViewDataSource,
                                       UITableViewDelegate>

enum { SECTION_FOLDERS, SECTION_FILES };
...
- (id)init {
  ...
  if (self) {
    ...
    NSString *host = [KeychainUtils stringForKey:KPT_FTP_KEY_HOST
                                  andServiceName:KPT_FTP_SERVICE];
    NSString *user = [KeychainUtils stringForKey:KPT_FTP_KEY_USER
                                  andServiceName:KPT_FTP_SERVICE];
    NSString *pw = [KeychainUtils stringForKey:KPT_FTP_KEY_PASSWD
                                andServiceName:KPT_FTP_SERVICE];

    self.client = [FTPClient clientWithHost:host
                                       port:21
                                   username:user
                                   password:pw];
  }
...
- (void)donePressed {
  ...
  for (NSString *aFilename in self.filesList) {
    if ([self.loadFilename compare:aFilename] == NSOrderedSame &&
        self.loadFilename.length == aFilename.length) {
      NSLog(@"File exists ");
      found = YES;
    }
  }
  if (found) {
    UIAlertController *alertCon = [UIAlertController
        alertControllerWithTitle:NSLocalizedString(@"Overwrite", nil)
...
- (void)startDownload {
  ...
  NSString *localPath =
      [documentsDirectory stringByAppendingPathComponent:self.loadFilename];

  [self.client
      downloadFile:[self.path stringByAppendingPathComponent:self.loadFilename]
      to:localPath
      progress:nil
      success:^{
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)startUpload {
  ...
  NSString *localPath =
      [documentsDirectory stringByAppendingPathComponent:self.loadFilename];
  NSString *uploadPath =
      [self.path stringByAppendingPathComponent:self.loadFilename];

  [self.client uploadFile:localPath
      to:uploadPath
      progress:nil
      success:^{
        [self removeLoadingAnimation];
...
- (void)tableView:(UITableView *)tableView
  ...
  switch (indexPath.section) {
  case SECTION_FOLDERS: {
    FTPSelectViewController *next =
        [[FTPSelectViewController alloc] initWithoutCancelWithMode:self.mode
                                                         andClient:self.client];
    next.loadFilename = self.loadFilename;
    next.path = [self.path
        stringByAppendingPathComponent:[tableView
  ...
  case SECTION_FILES: {
    if (self.mode == FTP_MODE_DOWNLOAD) {
      ...
      NSString *documentsDirectory =
          [KeePassTouchAppDelegate documentsDirectory];
      NSString *localPath =
          [documentsDirectory stringByAppendingPathComponent:self.loadFilename];
