...
- (void)viewDidLoad {
  ...
  [[self.userClient.filesRoutes listFolder:self.path]
      setResponseBlock:^(DBFILESListFolderResult *_Nullable result,
                         DBFILESListFolderError *_Nullable routeError,
        ...
        if (result) {
          ...
          if (hasMore) {
            ...
            if (self.folders.count == 0 && self.files.count == 0) {
              ...
              hud.label.text =
                  NSLocalizedString(@"No Folders or Files in here", nil);
            } else
            ...
            self.folders =
                [self.folders sortedArrayUsingDescriptors:@[ sortDescriptor ]];
            self.files =
                [self.files sortedArrayUsingDescriptors:@[ sortDescriptor ]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (indexPath.row < self.folders.count) {
    ...
    cell.textLabel.text =
        [self.files objectAtIndex:(indexPath.row - self.folders.count)].name;
...
- (void)tableView:(UITableView *)tableView
  ...
  if (indexPath.row < self.folders.count) {
    ...
    NSString *newPath = [[self.path
        stringByAppendingString:[tableView cellForRowAtIndexPath:indexPath]
                                    .textLabel.text]
        stringByAppendingString:@"/"];
    DropboxFolderController *dfc =
        [[DropboxFolderController alloc] initWithPath:newPath
                                             andStyle:self.tableView.style];
...
- (void)listFolderContinueWithCursor:(NSString *)cursor {
  [[self.userClient.filesRoutes listFolderContinue:cursor]
      setResponseBlock:^(DBFILESListFolderResult *response,
                         DBFILESListFolderContinueError *routeError,
...
- (void)addEntries:(NSArray<DBFILESMetadata *> *)entries {
  for (DBFILESMetadata *entry in entries) {
    if ([entry isKindOfClass:[DBFILESFileMetadata class]]) {
      ...
      if ([fileMetadata.name hasSuffix:@"kdb"] ||
          [fileMetadata.name hasSuffix:@"kdbx"])
...
- (IBAction)donePressed:(id)sender {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"DROPBOX_SYNC_NOTIFICATION"
                    object:nil];
