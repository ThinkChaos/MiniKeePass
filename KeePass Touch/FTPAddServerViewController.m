...
- (void)donePressed {
    ...
    NSString *fullHost = [host stringByAppendingString:[NSString stringWithFormat:@":%@",port]];
    FTPClient *client = [FTPClient clientWithHost:fullHost port:21 username:user password:password];
    
    [client listContentsAtPath:@"/" showHiddenFiles:NO success:^(NSArray *contents) {
        [KeychainUtils setString:fullHost forKey:KPT_FTP_KEY_HOST andServiceName:KPT_FTP_SERVICE];
        [KeychainUtils setString:_username.text forKey:KPT_FTP_KEY_USER andServiceName:KPT_FTP_SERVICE];
        [KeychainUtils setString:_password.text forKey:KPT_FTP_KEY_PASSWD andServiceName:KPT_FTP_SERVICE];
