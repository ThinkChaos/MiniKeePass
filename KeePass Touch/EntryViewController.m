...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        ...
        expireCell = [[TextFieldCell alloc] initWithStyle:UITableViewCellStyleValue2 reuseIdentifier:nil];
        expireCell.textLabel.text = NSLocalizedString(@"Expiration Date", nil);
        expireCell.textField.placeholder = NSLocalizedString(@"Expiration Date", nil);
        expireCell.textField.enabled = NO;
        expireCell.textField.autocorrectionType = UITextAutocorrectionTypeNo;
        expireCell.textField.autocapitalizationType = UITextAutocapitalizationTypeNone;
        ...
        _defaultCells = @[titleCell, usernameCell, passwordCell, expireCell, urlCell];
    ...
    if (editing == NO) {
        if (canceled) {
            ...
            for (UIViewController *con in self.navigationController.viewControllers) {
                if([NSStringFromClass(con.class) isEqualToString:@"GroupViewController"]) {
...
-(UIDocumentInteractionController *)documentInteractionControllerForURL:(NSURL *)fileURL {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        ...
        case SECTION_FILES: {
            ...
            // picture check
            if([ref.key hasSuffix:@".png"] ||
               [ref.key hasSuffix:@".jpg"] ||
               [ref.key hasSuffix:@".jpeg"] ||
               [ref.key hasSuffix:@".gif"]
               ) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.editing) {
        ...
        if(indexPath.section == SECTION_FILES) {
            ...
            Kdb4Tree *theTree = (Kdb4Tree*)[KeePassTouchAppDelegate appDelegate].databaseDocument.kdbTree;
            NSMutableArray *headerbins = theTree.dbVersion >= KDBX40_VERSION ? theTree.headerBinaries : theTree.binaries;
            ...
            if(theTree.dbVersion < KDBX40_VERSION) {
                ...
                if(attachment == nil) {
                    [[TWMessageBarManager sharedInstance] showMessageWithTitle:@"Error" description:@"failed to open document" type:TWMessageBarMessageTypeError duration:3.0 statusBarHidden:NO callback:nil];
                    [self.tableView deselectRowAtIndexPath:indexPath animated:YES];
                    return;
                }
                // decode base64 string to raw data
                base64DecodedData = [[NSData alloc] initWithBase64EncodedString:attachment.data options:NSDataBase64DecodingIgnoreUnknownCharacters];
            ...
            NSString *filePath = [tempDirectory stringByAppendingPathComponent:binaryReference.key];
...
- (void)copyCellContents:(NSIndexPath *)indexPath {
    ...
    CGSize size = [copiedString sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:font, NSFontAttributeName, nil]];
...
- (void)setSelectedImageIndex:(NSUInteger)index {
    ...
    UIImage *image = nil;
    if(self.isKdb4 && index == 0 && (((Kdb4Entry *)self.entry).customIconUuid != nil))
    {
...
- (void)imageButtonPressed {
    ...
    // Dann durchläuft ImageSelectionViewController alle Icons und zwar in der Reihenfolge der Keys durch ALLKEYS ARRAY und dann ImageforKey für einzelne Bilder und malt diese in Zeilen / Spalten
    // Wenn die Auswahl > als die Standardicons, also NUM_IMAGES sind rufe Delegate selectedImageCustomWithKey auf mit aus ALLKEYS Array Object Nummer OBJECTNUMBER_IN_TABLE - NUM_IMAGES ergibt 0 für erstes neues.
    //
...
- (void)imageSelectionView:(ImageSelectionView *)imageSelectionView selectedImageCustomWithKey:(NSString *)key
{
    ...
    customEntry.image = 0;
    customEntry.customIconUuid = [[KdbUUID alloc] initWithData:[KdbUUID stringToData:key]];
