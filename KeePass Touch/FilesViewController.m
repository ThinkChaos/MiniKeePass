...
- (void)viewDidLoad {
    ...
    if (@available(iOS 11.0, *)) {
        bottomLabelSpace += [[[UIApplication sharedApplication] delegate] window].safeAreaInsets.bottom;
    }
    
    footerLabel.frame = CGRectMake(0, self.view.bounds.size.height - self.navigationController.toolbar.frame.size.height - self.navigationController.navigationBar.frame.size.height - bottomLabelSpace, self.tableView.bounds.size.width, 30.0f);
...
/**
 @brief Block to be used when removing the loading animation and display an alert with this error<br>
 */
-(void (^) (NSError *error))showError {
    void (^ _Nonnull ret) (NSError * _Nullable error) = ^ (NSError *err) {
        ...
        {
            [self showAlertFromError:err completion:^{
                NSIndexPath *selPath = self.tableView.indexPathForSelectedRow;
                if(selPath)
                    [self.tableView deselectRowAtIndexPath:selPath animated:YES];
            }];
        }
    };
    return ret;
...
- (void)ftpPressed {
    ...
    if([KeychainUtils stringForKey:KPT_FTP_KEY_HOST andServiceName:KPT_FTP_SERVICE] == nil)
    {
...
- (void)syncToExtension {
    ...
    for (NSString *fileName in allFiles) {
        ...
        NSData *dbData = [NSData dataWithContentsOfFile:[[KeePassTouchAppDelegate documentsDirectory] stringByAppendingPathComponent:fileName]];
...
- (void)renameDatabase:(TextEntryController *)textEntryController {
    ...
    NSString *password = [KeychainUtils stringForKey:oldFilename andServiceName:KPT_PASSWORD_SERVICE];
    NSString *keyFile = [KeychainUtils stringForKey:oldFilename andServiceName:KPT_KEYFILES_SERVICE];
    
    // Store the password and keyfile into the keychain under the new filename
    [KeychainUtils setString:password forKey:newFilename andServiceName:KPT_PASSWORD_SERVICE];
    [KeychainUtils setString:keyFile forKey:newFilename andServiceName:KPT_KEYFILES_SERVICE];
    
    // Delete the keychain entries for the old filename
    [KeychainUtils deleteStringForKey:oldFilename andServiceName:KPT_PASSWORD_SERVICE];
    [KeychainUtils deleteStringForKey:oldFilename andServiceName:KPT_KEYFILES_SERVICE];
...
- (void)createNewDatabase:(NewKdbViewController *)newKdbViewController {
    NSInteger selectedSegment = newKdbViewController.versionSegmentedControl.selectedSegmentIndex;
    ...
    NSString *lowerCasePath = [documentsDirectory stringByAppendingPathComponent:filenameLowerCase];
    // Check if the file already exists
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:path] || [fileManager fileExistsAtPath:lowerCasePath]) {
    ...
    if(selectedSegment <= 2)
        [writer newFile:path withPassword:kdbPassword withDBVersion:selectedSegment];
    else {
        [newKdbViewController showErrorMessage:NSLocalizedString(@"Wrong Database Type selected", nil)];
        return;
    }
    
    // Store the password in the keychain
    if ([[AppSettings sharedInstance] rememberPasswordsEnabled]) {
        [KeychainUtils setString:password1 forKey:filename andServiceName:KPT_PASSWORD_SERVICE];
    }
...
- (void)openDatabaseWithFilename:(NSString *)filename {
    NSString *filePath = [[KeePassTouchAppDelegate documentsDirectory] stringByAppendingPathComponent:filename];
    [[DatabaseManager sharedInstance] openDatabaseDocument:filePath
        ...
        case SECTION_DATABASE:
            if (self.editing == NO) {
                ...
                if(_databaseFiles != nil && indexPath.row < _databaseFiles.count ) {
                    ...
                    NSString *filePath = [documentsDir stringByAppendingPathComponent:_databaseFiles[indexPath.row]];
                    [[DatabaseManager sharedInstance] openDatabaseDocument:filePath
                                                                   success:^(DatabaseDocument *doc) {
                                                                       self->appDelegate.databaseDocument = doc;
                                                                   }
                    ...
                    [self showErrorMessageOnHud:NSLocalizedString(@"Invalid database row", nil)];
            } else {
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        case SECTION_DATABASE:
            ...
            // Delete the keychain entries for the old filename
            [KeychainUtils deleteStringForKey:filename andServiceName:KPT_PASSWORD_SERVICE];
            [KeychainUtils deleteStringForKey:filename andServiceName:KPT_KEYFILES_SERVICE];
            break;
        case SECTION_KEYFILE:
...
#warning remove this after some versions
        ...
        NSString *alertMessage = @"Go ahead! Just open your database and you are good to go.\n\nYour passwords will now be securely available in any app.\n\nBe aware that the URL field is required to use it, so make use of it in as many entries as you can.";
        if(!purchased)
            alertMessage = @"Go ahead into settings and purchase Remove Ads + Auto Fill!\n\nAfter that, just open your database and you are good to go. Your passwords will now be securely available in any app.\n\nBe aware that the URL field is required to use it, so make use of it in as many entries as you can.";
        UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:@"Auto Fill is here!" message:alertMessage preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:nil];
        [alertCon addAction:cancelAction];
        [self presentViewController:alertCon animated:YES completion:nil];
