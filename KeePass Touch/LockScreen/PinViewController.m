...
#define PINTEXTFIELDWIDTH  51.0f
#define PINTEXTFIELDHEIGHT 42.0f
...
- (void)viewDidLoad {
    ...
    // Only if PIN enabled and not Touch ID
    ...
    //                          [self checkTouchID:success];
    //                          
    //                          // DISPLAY THE ERROR IN NSLOG
    //                          if(error != nil)
...
- (void)keyboardDidHide {
    //    AppSettings *appset = [AppSettings sharedInstance];
    //    if(![appset isTouchIDEnabled])
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
        [self resizeToolbarsToInterfaceOrientation:[[UIApplication sharedApplication] statusBarOrientation]];
    } completion:nil];
}
...
//- (void)checkTouchID:(BOOL)success {
//    if ([self.delegate respondsToSelector:@selector(pinViewController:touchIDEntered:)]) {
//        [self.delegate pinViewController:self touchIDEntered:YES];
//    }
//}
