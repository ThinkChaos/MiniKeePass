...
- (void)deleteKeychainData {
    ...
    // Delete all database passwords, keyfiles and ftp login data from the keychain
...
- (void)deleteAllData {
    ...
    NSArray *grpfiles = [fileManager contentsOfDirectoryAtPath:groupURL.absoluteString error:nil];
    for (NSString *file in grpfiles) {
        [fileManager removeItemAtPath:[groupURL.absoluteString stringByAppendingPathComponent:file] error:nil];
    }
...
{
    NSArray* tmpDirectoryFiles = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:NSTemporaryDirectory() error:NULL];
