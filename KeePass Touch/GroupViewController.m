...
- (void)viewWillAppear:(BOOL)animated {
    ...
    for (UIActivityIndicatorView *spinner in self.navigationController.view.subviews) {
        if(spinner.tag == 1007)
...
- (NSArray *)standardToolbarItems {
    if (_standardToolbarItems == nil) {
        ...
        UIBarButtonItem *changeMasterPassword = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"key"]
                                                                           style:UIBarButtonItemStylePlain
                                                                          target:self
        ...
        if(_group.parent == nil) {
            _standardToolbarItems = @[settingsButton, spacer, actionButton, spacer, changeMasterPassword, spacer, addButton];
        }
        else
...
//                                                            action:@selector(deleteSelectedItems)];
        self.deleteButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(deleteSelectedItems)];
...
//                                                          action:@selector(moveSelectedItems)];
        self.moveButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"move"] style:UIBarButtonItemStylePlain target:self action:@selector(moveSelectedItems)];
...
//                                                            action:@selector(renameSelectedItem)];
        self.renameButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"rename"] style:UIBarButtonItemStylePlain target:self action:@selector(renameSelectedItem)];
...
- (void)exportFilePressed:(id)sender {
    ...
    if (!didShow) {
        ...
        UIAlertController *actionSheetCon = [UIAlertController alertControllerWithTitle:prompt message:nil preferredStyle:UIAlertControllerStyleActionSheet];
        UIAlertAction *cancelOKAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"OK", nil) style:UIAlertActionStyleCancel handler:nil];
...
- (void)changeMasterPassword {
    ...
    kdbVC.donePressed = ^(FormViewController *formViewController) {
        NewKdbViewController *newKDBFinishedVC = (NewKdbViewController *)formViewController;
        
        if(newKDBFinishedVC.passwordTextField1.text.length == 0) {
            [formViewController showErrorMessage:NSLocalizedString(@"Password is required", nil)];
            return;
        }
        if(![newKDBFinishedVC.passwordTextField1.text isEqualToString:newKDBFinishedVC.passwordTextField2.text]) {
            [formViewController showErrorMessage:NSLocalizedString(@"Passwords do not match", nil)];
            return;
        }
        ...
        NSString *password = [KeychainUtils stringForKey:database.filename.lastPathComponent andServiceName:KPT_PASSWORD_SERVICE];
        
        // Store the new password inside
        if(password) {
            [KeychainUtils setString:newKDBFinishedVC.passwordTextField1.text forKey:database.filename.lastPathComponent andServiceName:KPT_PASSWORD_SERVICE];
        }
        [newKDBFinishedVC dismissViewControllerAnimated:YES completion:nil];
    };
    UINavigationController *navVC = [[UINavigationController alloc] initWithRootViewController:kdbVC];
...
- (void)addPressed {
    UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Add", nil) message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", nil) style:UIAlertActionStyleCancel handler:nil];
    UIAlertAction *groupAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"Group", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    if (self.group.canAddEntries) {
        UIAlertAction *entryAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"Entry", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    alertCon.popoverPresentationController.barButtonItem = [_standardToolbarItems lastObject];
...
- (void)deleteElementsFromModelAtIndexPaths:(NSArray *)indexPaths {
    ...
    {
        FilesViewController *fvc = (FilesViewController *)[self.navigationController.viewControllers objectAtIndex:0];
        [fvc syncDropbox];
    }
}
...
- (void)renameItem:(EditItemViewController *)editItemViewController {
    ...
    switch (indexPath.section) {
        case SECTION_GROUPS: {
            ...
            if(editItemViewController.isKdb4 && g.image == 0 && editItemViewController.searchKey != nil)
            {
                ((Kdb4Group *)g).customIconUuid = [[KdbUUID alloc] initWithData:[KdbUUID stringToData:editItemViewController.searchKey]];
            }
            break;
        }
        ...
        case SECTION_ENTRIES: {
            ...
            if(editItemViewController.isKdb4 && e.image == 0 &&  editItemViewController.searchKey != nil)
            {
                ((Kdb4Entry *)e).customIconUuid = [[KdbUUID alloc] initWithData:[KdbUUID stringToData:editItemViewController.searchKey]];
            }
            break;
        }
    }
        ...
        [store getCredentialIdentityStoreStateWithCompletion:^(ASCredentialIdentityStoreState * _Nonnull state) {
            if([state isEnabled]) {
                [[ASCredentialIdentityStore sharedStore] removeAllCredentialIdentitiesWithCompletion:^(BOOL success, NSError * _Nullable error) {
                    if(success) {
                        NSArray <ASPasswordCredentialIdentity *> *credentials = [NSArray array];
                        NSArray <KdbEntry *> *arr = _group.allEntries;
                        for (KdbEntry *entry in arr) {
                            if(entry.url.length > 0) {
                                ASCredentialServiceIdentifier *identifier = [[ASCredentialServiceIdentifier alloc] initWithIdentifier:entry.url type:ASCredentialServiceIdentifierTypeURL];
                                ASPasswordCredentialIdentity *identity = [[ASPasswordCredentialIdentity alloc] initWithServiceIdentifier:identifier user:entry.username recordIdentifier:entry.recordIdentifier];
                                credentials = [credentials arrayByAddingObject:identity];
                            }
                        }
                        [store saveCredentialIdentities:credentials completion:^(BOOL success, NSError * _Nullable error) {
                            if(success) {
#ifdef DEBUG
