...
- (instancetype)initWithEntries:(NSArray<KdbEntry *> *)entries
               andSearchStrings:(NSArray<NSString *> *)searchStrings {
  self = [super init];
...
- (void)createLikelyInitials:(NSArray<NSString *> *)searchStrings {
  ...
  for (NSString *searchParticle in searchStrings) {
    likelyEntries = [likelyEntries
        arrayByAddingObjectsFromArray:[DatabaseDocument
                                          filterEntries:_initialEntries
                                             searchText:searchParticle]];
  }
  NSSet *likelySet = [NSSet setWithArray:likelyEntries];
  likelyEntries = [likelySet allObjects];

  // create likelies
  NSMutableDictionary<NSNumber *, NSArray *> *createLikelies =
...
- (NSArray *)likelyEntries {
  if (_likelyRankedEntries.allValues.count == 0) {
    ...
    for (NSNumber *rank in _likelyInitials.allKeys) {
      ...
      rankEntries = [DatabaseDocument filterEntries:rankEntries
                                         searchText:_currentSearchString];
      [_likelyRankedEntries setObject:rankEntries forKey:rank];
    }
  }
  NSArray *keys = [_likelyRankedEntries allKeys];
  keys = [keys sortedArrayUsingComparator:^NSComparisonResult(
  ...
  for (int i = 0; i < keys.count; i++) {
    sortedEntries =
        [sortedEntries arrayByAddingObjectsFromArray:[_likelyRankedEntries
                                                         objectForKey:keys[i]]];
  }
...
- (NSArray *)otherEntries {
  if (_entries.count == 0) {
    // build entries
    _entries = [DatabaseDocument filterEntries:_initialEntries
                                    searchText:_currentSearchString];
  }
  return _entries;
}
