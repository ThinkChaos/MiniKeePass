...
- (id)initWithReadIO:(xmlInputReadCallback)ioread
             ...
             options:(NSUInteger)mask
               error:(NSError **)error {
  // Even though xmlKeepBlanksDefault(0) is called in DDXMLNode's initialize
  // method, it has been documented that this call seems to get reset on the
  // iPhone: http://code.google.com/p/kissxml/issues/detail?id=8
  //
  ...
  if (doc == NULL) {
    if (error) {
      *error = [NSError errorWithDomain:@"DDXMLErrorDomain"
                                   code:1
                               userInfo:nil];
    }
