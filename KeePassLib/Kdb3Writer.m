...
- (void)writeGroups:(Kdb3Group *)root
    withOutputStream:(OutputStream *)outputStream;
- (void)writeEntries:(Kdb3Group *)root
    withOutputStream:(OutputStream *)outputStream;
- (void)writeMetaEntries:(Kdb3Group *)root
        withOutputStream:(OutputStream *)outputStream;
...
- (void)appendField:(uint16_t)type
                size:(uint32_t)size
 ...
 */
- (void)persist:(Kdb3Tree *)tree
            file:(NSString *)filename
    withPassword:(KdbPassword *)kdbPassword {
  FileOutputStream *fileOutputStream =
      [[FileOutputStream alloc] initWithFilename:filename
  ...
  if (tree.flags & FLAG_RIJNDAEL) {
    encryptedStream =
        [[AesOutputStream alloc] initWithOutputStream:fileOutputStream
                                                  key:key
                                                   iv:encryptionIv];
  } else if (tree.flags & FLAG_TWOFISH) {
    encryptedStream =
        [[TwoFishOutputStream alloc] initWithOutputStream:fileOutputStream
                                                      key:key
                                                       iv:encryptionIv];
  }

  // Wrap the AES output stream in a SHA256 output stream to calculate the
  // content hash
  Sha256OutputStream *shaOutputStream =
      [[Sha256OutputStream alloc] initWithOutputStream:encryptedStream];
  ...
  @try {
    ...
    fileOutputStream = [[FileOutputStream alloc] initWithFilename:filename
                                                            flags:O_WRONLY
                                                             mode:0644];
...
- (void)writeHeader:(OutputStream *)outputStream withTree:(Kdb3Tree *)tree {
  ...
  [encryptionIv getBytes:header.encryptionIv
  ...
  // Skip the content hash for now, it will get filled in after the content is
  // written
...
- (void)writeGroup:(Kdb3Group *)group
  ...
  if (firstGroup) {
    ...
    [self appendField:0
                    size:(uint32_t)dataOutputStream.data.length
                   bytes:dataOutputStream.data.bytes
...
- (void)writeEntry:(Kdb3Entry *)entry
  ...
  if (entry.binary && entry.binary.length) {
    [self appendField:14
                    size:(uint32_t)entry.binary.length
                   bytes:entry.binary.bytes
        withOutputStream:outputStream];
  } else {
...
- (void)writeExtData:(OutputStream *)outputStream {
  ...
  [self appendField:1
                  size:32
                 bytes:headerHash.bytes
      withOutputStream:outputStream];

  // Generate some random data to prevent guessing attacks that use the content
  // hash
  NSData *randomData = [Utils randomBytes:32];
  [self appendField:2
                  size:32
                 bytes:randomData.bytes
      withOutputStream:outputStream];
