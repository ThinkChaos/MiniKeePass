...
- (KDB4CustomIcon *)parseCustomIcon:(DDXMLElement *)root;
...
- (Kdb4Tree *)parse:(InputStream *)inputStream dbVersion:(uint32_t)dbVer {
  ...
  for (DDXMLElement *deletedObjectElement in
       [deletedObjects elementsForName:@"DeletedObject"]) {
    DeletedObject *deletecObject =
        [self parseDeletedObject:deletedObjectElement];
...
- (void)parseMeta:(DDXMLElement *)root tree:(Kdb4Tree *)tree {
  ...
  tree.databaseNameChanged = [self
      parseDateTime:[[root elementForName:@"DatabaseNameChanged"] stringValue]];
  tree.databaseDescription =
      [[root elementForName:@"DatabaseDescription"] stringValue];
  tree.databaseDescriptionChanged =
      [self parseDateTime:[[root elementForName:@"DatabaseDescriptionChanged"]
                              stringValue]];
  tree.defaultUserName = [[root elementForName:@"DefaultUserName"] stringValue];
  tree.defaultUserNameChanged =
      [self parseDateTime:[[root elementForName:@"DefaultUserNameChanged"]
                              stringValue]];
  tree.maintenanceHistoryDays =
      [[[root elementForName:@"MaintenanceHistoryDays"] stringValue]
          integerValue];
  tree.color = [[root elementForName:@"Color"] stringValue];
  tree.masterKeyChanged = [self
      parseDateTime:[[root elementForName:@"MasterKeyChanged"] stringValue]];
  tree.masterKeyChangeRec =
      [[[root elementForName:@"MasterKeyChangeRec"] stringValue] integerValue];
  tree.masterKeyChangeForce = [[[root elementForName:@"MasterKeyChangeForce"]
      stringValue] integerValue];

  DDXMLElement *memoryProtectionElement =
      [root elementForName:@"MemoryProtection"];
  tree.protectTitle = [[[memoryProtectionElement elementForName:@"ProtectTitle"]
      stringValue] boolValue];
  tree.protectUserName =
      [[[memoryProtectionElement elementForName:@"ProtectUserName"] stringValue]
          boolValue];
  tree.protectPassword =
      [[[memoryProtectionElement elementForName:@"ProtectPassword"] stringValue]
          boolValue];
  tree.protectUrl = [[[memoryProtectionElement elementForName:@"ProtectURL"]
      stringValue] boolValue];
  tree.protectNotes = [[[memoryProtectionElement elementForName:@"ProtectNotes"]
      stringValue] boolValue];
  ...
  //    initializeWithCustomIcons:tree.customIcons];

  tree.recycleBinEnabled =
      [[[root elementForName:@"RecycleBinEnabled"] stringValue] boolValue];
  tree.recycleBinUuid = [self
      parseUuidString:[[root elementForName:@"RecycleBinUUID"] stringValue]];
  tree.recycleBinChanged = [self
      parseDateTime:[[root elementForName:@"RecycleBinChanged"] stringValue]];
  tree.entryTemplatesGroup =
      [self parseUuidString:[[root elementForName:@"EntryTemplatesGroup"]
                                stringValue]];
  tree.entryTemplatesGroupChanged =
      [self parseDateTime:[[root elementForName:@"EntryTemplatesGroupChanged"]
                              stringValue]];
  tree.historyMaxItems =
      [[[root elementForName:@"HistoryMaxItems"] stringValue] integerValue];
  tree.historyMaxSize =
      [[[root elementForName:@"HistoryMaxSize"] stringValue] integerValue];
  tree.lastSelectedGroup = [self
      parseUuidString:[[root elementForName:@"LastSelectedGroup"] stringValue]];
  tree.lastTopVisibleGroup =
      [self parseUuidString:[[root elementForName:@"LastTopVisibleGroup"]
                                stringValue]];
...
- (KDB4CustomIcon *)parseCustomIcon:(DDXMLElement *)root {
  KDB4CustomIcon *customIcon = [[KDB4CustomIcon alloc] init];

  customIcon.uuid =
      [self parseUuidString:[[root elementForName:@"UUID"] stringValue]];
...
- (Binary *)parseBinary:(DDXMLElement *)root {
  ...
  binary.compressed =
      [[[root attributeForName:@"Compressed"] stringValue] boolValue];
...
- (Kdb4Group *)parseGroup:(DDXMLElement *)root {
  ...
  if (customIconUuidElement != nil) {
    //        NSString *representingCustomIcon = [customIconUuidElement
    //        stringValue];
    group.customIconUuid =
        [self parseUuidString:[customIconUuidElement stringValue]];
    //        NSLog(@"group.customIconUuid is %@", representingCustomIcon);
    //        for (NSString *str in customIco) {
    //            NSString *stringtest = [[NSString alloc]
    //            initWithData:group.customIconUuid.getData
    //            encoding:NSASCIIStringEncoding]; if([representingCustomIcon
    //            compare:str] == NSOrderedSame)
    //            {
    //                [group.customIconUuid.getData isEqualToData:[[self
    //                parseUuidString:[customIconUuidElement stringValue]]
    //                getData]]; if([group.customIconUuid.getData
    //                isEqualToData:[[self
    //                parseUuidString:[customIconUuidElement stringValue]]
    //                getData]])
    //                    NSLog(@"DATEN GLEICH");
    ...
    //                NSLog(@"Treffer");
    //                NSLog(@"-------");
    //                NSLog(@"custom icon %@", str);
    //                NSLog(@"representinggrpicon %@", representingCustomIcon);
    //
    //            }
    //            else if([str compare:stringtest] == NSOrderedSame) {
    //                NSLog(@"sind die gleich %@ und %@", stringtest, str);
    //                NSLog(@"JAAAA!!!");
    //            }
    //        }
  }
  ...
  group.lastModificationTime =
      [self parseDateTime:[[timesElement elementForName:@"LastModificationTime"]
                              stringValue]];
  group.creationTime =
      [self parseDateTime:[[timesElement elementForName:@"CreationTime"]
                              stringValue]];
  group.lastAccessTime =
      [self parseDateTime:[[timesElement elementForName:@"LastAccessTime"]
                              stringValue]];
  group.expiryTime = [self
      parseDateTime:[[timesElement elementForName:@"ExpiryTime"] stringValue]];
  group.expires =
      [[[timesElement elementForName:@"Expires"] stringValue] boolValue];
  group.usageCount =
      [[[timesElement elementForName:@"UsageCount"] stringValue] integerValue];
  group.locationChanged =
      [self parseDateTime:[[timesElement elementForName:@"LocationChanged"]
                              stringValue]];

  group.isExpanded =
      [[[root elementForName:@"IsExpanded"] stringValue] boolValue];
  group.defaultAutoTypeSequence =
      [[root elementForName:@"DefaultAutoTypeSequence"] stringValue];
  group.enableAutoType = [[root elementForName:@"EnableAutoType"] stringValue];
  group.enableSearching =
      [[root elementForName:@"EnableSearching"] stringValue];
  group.lastTopVisibleEntry =
      [self parseUuidString:[[root elementForName:@"LastTopVisibleEntry"]
                                stringValue]];
...
- (Kdb4Entry *)parseEntry:(DDXMLElement *)root {
  ...
  entry.foregroundColor =
      [[root elementForName:@"ForegroundColor"] stringValue];
  entry.backgroundColor =
      [[root elementForName:@"BackgroundColor"] stringValue];
...
- (StringField *)parseStringField:(DDXMLElement *)root {
  ...
  stringField.protected =
      [[[valueElement attributeForName:@"Protected"] stringValue] boolValue];
...
- (BinaryRef *)parseBinaryRef:(DDXMLElement *)root {
  ...
  binaryRef.ref = [[[[root elementForName:@"Value"] attributeForName:@"Ref"]
      stringValue] integerValue];
...
- (AutoType *)parseAutoType:(DDXMLElement *)root {
  ...
  autoType.dataTransferObfuscation =
      [[[root elementForName:@"DataTransferObfuscation"] stringValue]
          integerValue];

  DDXMLElement *defaultSequenceElement =
      [root elementForName:@"DefaultSequence"];
  ...
  for (DDXMLElement *element in [root elementsForName:@"Association"]) {
    ...
    association.keystrokeSequence =
        [[element elementForName:@"KeystrokeSequence"] stringValue];
...
- (NSDate *)parseDateTime:(NSString *)dateString {
  ...
  if (dbVersion >= KDBX40_VERSION) {
    NSData *dateBytes = [[NSData alloc]
        initWithBase64EncodedString:dateString
                            options:
...
- (DeletedObject *)parseDeletedObject:(DDXMLElement *)root {
  ...
  deletedObject.uuid =
      [self parseUuidString:[[root elementForName:@"UUID"] stringValue]];
  deletedObject.deletionTime =
      [self parseDateTime:[[root elementForName:@"DeletionTime"] stringValue]];
