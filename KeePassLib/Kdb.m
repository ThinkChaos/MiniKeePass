...
- (NSArray *)allEntries {
    if(self.groups.count == 0) {
        ...
        for (KdbGroup *subGroup in self.groups) {
            allEntries = [allEntries arrayByAddingObjectsFromArray:[subGroup allEntries]];
        }
        return allEntries;
    }
}
...
- (NSInteger)rankForSearchString:(NSString *)searchString {
    ...
    else if([self.title rangeOfString:searchString options:NSCaseInsensitiveSearch].length > 0)
        rank+=searchString.length;
    
    if([self.url isEqualToString:searchString])
        rank+=searchString.length*2;
    else if([self.url rangeOfString:searchString options:NSCaseInsensitiveSearch].length > 0)
        rank+=searchString.length;
    
    if([self.notes isEqualToString:searchString])
        rank+=2;
    else if([self.notes rangeOfString:searchString options:NSCaseInsensitiveSearch].length > 0)
        rank++;
...
- (NSString *)recordIdentifier {
    return [NSString stringWithFormat:@"%@:%@:%@", self.parent.name, self.title, self.creationTime];
}
