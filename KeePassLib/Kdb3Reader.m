...
- (KdbTree*)load:(InputStream *)inputStream withPassword:(KdbPassword *)kdbPassword {
    ...
    if( headerFlags & FLAG_RIJNDAEL ) {
        decryptStream = [[AesInputStream alloc] initWithInputStream:inputStream key:key iv:encryptionIv];
    } else if( headerFlags & FLAG_TWOFISH ) {
        decryptStream = [[TwoFishInputStream alloc] initWithInputStream:inputStream key:key iv:encryptionIv];
    }
...
- (void)readGroups:(InputStream *)inputStream {
    ...
    for (uint32_t i = 0; i < numGroups; i++) {
        ...
        // Parse the fields
        eos = NO;
        // use whileCounter to detect first default case -> wrong password -> db not readable
        ...
        while (!eos) {
            ...
            switch (fieldType) {
                ...
                default: {
                    if(whileCounter == 1)
                        @throw [NSException exceptionWithName:@"IOException" reason:@"Wrong Password" userInfo:nil];
                    else
