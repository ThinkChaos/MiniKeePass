...
- (KdbTree *)load:(InputStream *)inputStream
  ...
  NSData *key = [kdbPassword createFinalKeyForVersion:3
                                           masterSeed:masterSeed
                                        transformSeed:masterSeed2
                                               rounds:keyEncRounds];

  InputStream *decryptStream;
  if (headerFlags & FLAG_RIJNDAEL) {
    decryptStream = [[AesInputStream alloc] initWithInputStream:inputStream
                                                            key:key
                                                             iv:encryptionIv];
  } else if (headerFlags & FLAG_TWOFISH) {
    decryptStream =
        [[TwoFishInputStream alloc] initWithInputStream:inputStream
                                                    key:key
                                                     iv:encryptionIv];
  }
  ...
  @try {
    // Parse groups
    [self readGroups:decryptStream];

    // Parse entries
    [self readEntries:decryptStream];
...
- (void)readHeader:(InputStream *)inputStream {
  ...
  masterSeed = [[NSData alloc] initWithBytes:header.masterSeed
                                      length:sizeof(header.masterSeed)];
  encryptionIv = [[NSData alloc] initWithBytes:header.encryptionIv
                                        length:sizeof(header.encryptionIv)];

  numGroups = CFSwapInt32LittleToHost(header.groups);
  numEntries = CFSwapInt32LittleToHost(header.entries);

  contentsHash = [[NSData alloc] initWithBytes:header.contentsHash
                                        length:sizeof(header.contentsHash)];
  masterSeed2 = [[NSData alloc] initWithBytes:header.masterSeed2
                                       length:sizeof(header.masterSeed2)];

  keyEncRounds = CFSwapInt32LittleToHost(header.keyEncRounds);

  // Compute a sha256 hash of the header up to but not including the
  // contentsHash
...
- (void)readGroups:(InputStream *)inputStream {
  ...
  for (uint32_t i = 0; i < numGroups; i++) {
    ...
    // Parse the fields
    eos = NO;
    // use whileCounter to detect first default case -> wrong password -> db not
    // readable
    ...
    while (!eos) {
      ...
      case 0x0002:
        group.name = [inputStream readCString:fieldSize
      ...
      case 0x0007:
        ...
        NSLog(@"KDB3 Image: %ld", (long)group.image);
        break;
...
- (void)readEntries:(InputStream *)inputStream {
  ...
  for (uint32_t i = 0; i < numEntries; i++) {
    ...
    while (!eos) {
      ...
      case 0x0004:
        entry.title = [inputStream readCString:fieldSize
                                      encoding:NSUTF8StringEncoding];
        break;

      case 0x0005:
        entry.url = [inputStream readCString:fieldSize
                                    encoding:NSUTF8StringEncoding];
        break;

      case 0x0006:
        entry.username = [inputStream readCString:fieldSize
                                         encoding:NSUTF8StringEncoding];
        break;

      case 0x0007:
        entry.password = [inputStream readCString:fieldSize
                                         encoding:NSUTF8StringEncoding];
        break;

      case 0x0008:
        entry.notes = [inputStream readCString:fieldSize
                                      encoding:NSUTF8StringEncoding];
        break;
      ...
      case 0x000D:
        entry.binaryDesc = [inputStream readCString:fieldSize
                                           encoding:NSUTF8StringEncoding];
        break;
  ...
  while (!eos) {
    ...
    case 0x0001:
      ...
      if (memcmp(headerHash.bytes, buffer, fieldSize) != 0) {
        @throw [NSException exceptionWithName:@"IOException"
                                       reason:@"Header hash does not match"
                                     userInfo:nil];
      }
      break;
...
- (Kdb3Tree *)buildTree {
  ...
  for (i = 0; i < [groups count]; i++) {
    ...
    for (j = i - 1; j >= 0; j--) {
      ...
      if (level2 < level1) {
        if (level1 - level2 != 1) {
          @throw [NSException exceptionWithName:@"InvalidData"
                                         reason:@"InvalidTree"
                                       userInfo:nil];
        } else {
          break;
