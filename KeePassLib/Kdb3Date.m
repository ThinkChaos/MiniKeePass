...
+ (NSDate *)fromPacked:(uint8_t *)buffer {
  ...
  dw1 = (uint32_t)buffer[0];
  dw2 = (uint32_t)buffer[1];
  dw3 = (uint32_t)buffer[2];
  dw4 = (uint32_t)buffer[3];
  dw5 = (uint32_t)buffer[4];
  int y = (dw1 << 6) | (dw2 >> 2);
...
+ (void)toPacked:(NSDate *)date bytes:(uint8_t *)bytes {
  ...
  bytes[2] = (uint8_t)(((mon & 0x00000003) << 6) | ((d & 0x0000001F) << 1) |
                       ((h >> 4) & 0x00000001));
