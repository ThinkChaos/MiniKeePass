...
- (KdbTree *)load:(FileInputStream *)inputStream
  ...
  NSData *key = [kdbPassword createFinalKeyKDBX4:kdfParams
                                      masterSeed:mseed
                                       HmacKey64:hmackey64];
  ...
  if (dbVersion < VERSION_CRITICAL_MAX_32_4) { // KDBX 3.1
    ...
    stream = [CipherStreamFactory getInputStream:cipherUuid
                                          stream:inputStream
                                             key:key
                                              iv:encryptionIv];
    ...
    if (memcmp(headerReadHash, headerStoredHash.bytes, 32) != 0) {
      @throw [NSException exceptionWithName:@"IOFileCorrupt"
                                     reason:@"Header data corrupt"
                                   userInfo:nil];
    }
    ...
    CCHmac(kCCHmacAlgSHA256, hmacKey.bytes, hmacKey.length, headerBytes.bytes,
           (size_t)headerBytes.length, headerReadHmac);
    [inputStream read:headerStoredHmac length:32];
    if (memcmp(headerReadHmac, headerStoredHmac, 32) != 0) {
      @throw [NSException
          exceptionWithName:@"IOFileCorrupt"
                     reason:@"Password incorrect / Header data corrupt"
                   userInfo:nil];
    }

    // Create the HMAC Block input stream
    NSData *hmacKeyData = [[NSData alloc] initWithBytes:hmackey64 length:64];
    HmacInputStream *hmacStream =
        [[HmacInputStream alloc] initWithInputStream:inputStream
                                                 key:hmacKeyData];
  ...
  if (protectedStreamKey == nil) {
    @throw
        [NSException exceptionWithName:@"IOException"
                                reason:@"Inner Protected Stream Key NOT FOUND."
                              userInfo:nil];
  }
  ...
  if (randomStreamID == CSR_SALSA20) {
    ...
    @throw [NSException exceptionWithName:@"IOException"
                                   reason:@"Unsupported CSR algorithm"
                                 userInfo:nil];
  }
...
- (void)readHeader:inputStream {
  ...
  if ((dbVersion & VERSION_CRITICAL_MASK) >
      (VERSION_CRITICAL_MAX_32_4 & VERSION_CRITICAL_MASK)) {
    @throw [NSException exceptionWithName:@"IOException"
  ...
  while (!eoh) {
    ...
    case HEADER_TRANSFORMSEED: // Obsolete in KDBX 4
      ...
      if (kdfParams[KDF_KEY_UUID_BYTES] == nil) {
        [kdfParams addByteArray:[[KdbUUID getAES_KDFUUID] getData]
                         forKey:KDF_KEY_UUID_BYTES];
      }
      [kdfParams addByteArray:[inputStream readData:fieldSize]
    ...
    default:
      @throw [NSException exceptionWithName:@"InvalidHeader"
                                     reason:@"InvalidField"
                                   userInfo:nil];
    }
  }
...
- (void)readInnerHeader:inputStream {
  ...
  while (!eoh) {
    ...
    case INNER_HEADER_BINARY:
      ...
      break;
    default:
      @throw [NSException exceptionWithName:@"InvalidParameterField"
                                     reason:@"Inner Header BadFieldType"
                                   userInfo:nil];
    }
  }
}
