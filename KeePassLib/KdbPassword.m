...
- (id)copyWithNewPassword:(NSString *)newPW {
  return [[KdbPassword alloc] initWithPassword:newPW
                              passwordEncoding:passswordEncoding
                                       keyFile:keyFile];
}
...
- (NSData *)createFinalKeyForVersion:(uint8_t)version
  ...
  if (CCCryptorCreate(kCCEncrypt, kCCAlgorithmAES128, kCCOptionECBMode,
                      transformSeed.bytes, kCCKeySizeAES256, nil,
                      &cryptorRef) != kCCSuccess) {
    @throw [NSException exceptionWithName:@"CryptoException"
                                   reason:@"Failed create ref"
                                 userInfo:nil];
  };
...
- (NSData *)createFinalKeyKDBX4:(VariantDictionary *)kdfparams
  ...
  if ([uuid isEqual:[KdbUUID getAES_KDFUUID]]) {
    ...
    uint32_t parallelism =
        [kdfparams[KDF_ARGON2_KEY_PARALLELISM] unsignedIntValue];
    uint8_t *salt = (uint8_t *)[kdfparams[KDF_ARGON2_KEY_SALT] bytes];
    uint32_t saltlen = (uint32_t)[kdfparams[KDF_ARGON2_KEY_SALT] length];
    uint8_t result[32];
    argon2d_hash_raw(t_cost, (uint32_t)m_cost / 1024, parallelism, masterKey,
                     32, salt, saltlen, result, 32);
...
+ (void)checkAESParameters:(VariantDictionary *)kdf {
  ...
  if (kdf[KDF_AES_KEY_SEED] == nil) {
    @throw [NSException exceptionWithName:@"CryptoException"
                                   reason:@"AES seed not set"
                                 userInfo:nil];
  } else {
    if ([kdf[KDF_AES_KEY_SEED] length] != 32) {
      @throw [NSException exceptionWithName:@"CryptoException"
                                     reason:@"AES seed length error"
                                   userInfo:nil];
    }
  }
}
...
+ (void)checkArgon2Parameters:(VariantDictionary *)kdf {
  ...
  if (kdf[KDF_ARGON2_KEY_ITERATIONS] == nil) {
    @throw [NSException exceptionWithName:@"CryptoException"
                                   reason:@"Argon2 iterations not set"
                                 userInfo:nil];
  }

  if (kdf[KDF_ARGON2_KEY_MEMORY] == nil) {
    @throw [NSException exceptionWithName:@"CryptoException"
                                   reason:@"Argon2 memory not set"
                                 userInfo:nil];
  }

  if (kdf[KDF_ARGON2_KEY_PARALLELISM] == nil) {
    @throw [NSException exceptionWithName:@"CryptoException"
                                   reason:@"Argon2 parallelism not set"
                                 userInfo:nil];
  } else {
    if ([(NSNumber *)kdf[KDF_ARGON2_KEY_PARALLELISM] unsignedLongLongValue] <=
        0) {
      @throw [NSException exceptionWithName:@"CryptoException"
                                     reason:@"Argon2 parallelism bad value"
                                   userInfo:nil];
    }
  }
  ...
  if (kdf[KDF_ARGON2_KEY_SALT] == nil) {
    @throw [NSException exceptionWithName:@"CryptoException"
                                   reason:@"Argon2 salt not set"
                                 userInfo:nil];
  } else {
    if ([kdf[KDF_ARGON2_KEY_SALT] length] != 32) {
      @throw [NSException exceptionWithName:@"CryptoException"
                                     reason:@"Argon2 salt not 32 bytes"
                                   userInfo:nil];
    }
  }
}
...
+ (VariantDictionary *)getDefaultKDFParameters:(KdbUUID *)uuid {
  ...
  if ([uuid isEqual:[KdbUUID getAES_KDFUUID]]) {
    ...
    [kdf addUInt32:DEFAULT_ARGON2_PARALLELISM
            forKey:KDF_ARGON2_KEY_PARALLELISM];
...
- (NSData *)loadKeyFileV4:(NSString *)filename {
  ...
  @try {
    ...
    // Ignore the exception and try and load the file through a different
    // mechanism    NSData *data = nil;
  }
...
- (NSData *)loadXmlKeyFile:(NSString *)filename {
  NSString *xmlString = [NSString stringWithContentsOfFile:filename
                                                  encoding:NSUTF8StringEncoding
                                                     error:nil];
  if (xmlString == nil) {
    @throw [NSException exceptionWithName:@"IOException"
                                   reason:@"Failed to open keyfile"
                                 userInfo:nil];
  }

  DDXMLDocument *document = [[DDXMLDocument alloc] initWithXMLString:xmlString
                                                             options:0
                                                               error:nil];
  if (document == nil) {
    @throw [NSException exceptionWithName:@"ParseError"
                                   reason:@"Failed to parse keyfile"
                                 userInfo:nil];
  }
