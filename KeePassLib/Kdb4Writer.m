...
- (void)writeHeaderField:(OutputStream *)outputStream
                headerId:(uint8_t)headerId
...
- (void)persist:(Kdb4Tree *)tree
  ...
  KdbUUID *KDFUuid =
      [[KdbUUID alloc] initWithData:tree.kdfParams[KDF_KEY_UUID_BYTES]];
  if ([KDFUuid isEqual:[KdbUUID getAES_KDFUUID]]) {
    [tree.kdfParams addByteArray:[Utils randomBytes:32]
                          forKey:KDF_AES_KEY_SEED];
  } else if ([KDFUuid isEqual:[KdbUUID getArgon2UUID]]) {
    [tree.kdfParams addByteArray:[Utils randomBytes:32]
                          forKey:KDF_ARGON2_KEY_SALT];
  } else {
    @throw [NSException exceptionWithName:@"CipherError"
                                   reason:@"Unknown Cipher Uuid"
                                 userInfo:nil];
  }
  ...
  NSData *key = [kdbPassword createFinalKeyKDBX4:tree.kdfParams
                                      masterSeed:mseed
                                       HmacKey64:hmackey64];
  OutputStream *stream;
  RandomStream *randomStream;
  if (dbVersion < KDBX40_VERSION) { // KDBX 3.1
    stream = [CipherStreamFactory getOutputStream:tree.encryptionAlgorithm
                                           stream:outputStream
                                              key:key
                                               iv:encryptionIv];
    ...
    stream = [[HashedOutputStream alloc] initWithOutputStream:stream
                                                    blockSize:1024 * 1024];
    ...
    NSData *hmacKey = [HmacInputStream getHMACKey:(void *)hmackey64
                                           keylen:64
                                       blockIndex:ULLONG_MAX];
    CCHmac(kCCHmacAlgSHA256, hmacKey.bytes, hmacKey.length, headerBytes.bytes,
           (size_t)headerBytes.length, headerHmac);
    ...
    HmacOutputStream *hmacStream =
        [[HmacOutputStream alloc] initWithOutputStream:outputStream
                                                   key:hmacKeyData];
  ...
  // Serialize the XML
  Kdb4Persist *persist = [[Kdb4Persist alloc] initWithTree:tree
                                              outputStream:stream
                                              randomStream:randomStream];
  [persist persist];
  ...
  if (![outputStream.data writeToFile:filename
...
- (void)writeHeader:(OutputStream *)outputStream withTree:(Kdb4Tree *)tree {
  ...
  [self writeHeaderField:outputStream
                headerId:HEADER_CIPHERID
                    data:buffer
                  length:16];

  i32 = CFSwapInt32HostToLittle(tree.compressionAlgorithm);
  [self writeHeaderField:outputStream
                headerId:HEADER_COMPRESSION
                    data:&i32
                  length:4];

  [self writeHeaderField:outputStream
                headerId:HEADER_MASTERSEED
                    data:masterSeed.bytes
                  length:masterSeed.length];

  if (dbVersion < KDBX40_VERSION) {
    NSData *seedData = (NSData *)tree.kdfParams[KDF_AES_KEY_SEED];

    [self writeHeaderField:outputStream
                  headerId:HEADER_TRANSFORMSEED
                      data:seedData.bytes
                    length:seedData.length];

    uint64_t rounds =
        [tree.kdfParams[KDF_AES_KEY_ROUNDS] unsignedLongLongValue];
    i64 = CFSwapInt64HostToLittle(rounds);
    [self writeHeaderField:outputStream
                  headerId:HEADER_TRANSFORMROUNDS
                      data:&i64
                    length:8];
  } else {
    NSData *vdBytes = [tree.kdfParams serialize];
    [self writeHeaderField:outputStream
                  headerId:HEADER_KDFPARMETERS
                      data:vdBytes.bytes
                    length:vdBytes.length];
  }

  if (encryptionIv.length > 0) {
    [self writeHeaderField:outputStream
                  headerId:HEADER_ENCRYPTIONIV
                      data:encryptionIv.bytes
                    length:encryptionIv.length];
  }

  if (dbVersion < KDBX40_VERSION) {
    [self writeHeaderField:outputStream
                  headerId:HEADER_PROTECTEDKEY
                      data:protectedStreamKey.bytes
                    length:protectedStreamKey.length];

    [self writeHeaderField:outputStream
                  headerId:HEADER_STARTBYTES
                      data:streamStartBytes.bytes
                    length:streamStartBytes.length];

    i32 = CFSwapInt32HostToLittle(CSR_SALSA20);
    [self writeHeaderField:outputStream
                  headerId:HEADER_RANDOMSTREAMID
                      data:&i32
                    length:4];
  } else {
...
- (void)writeInnerHeader:(OutputStream *)outputStream
                withTree:(Kdb4Tree *)tree {
  uint8_t buffer[16];
  uint32_t i32;
  ...
  [self writeHeaderField:outputStream
                headerId:INNER_HEADER_RANDOMSTREAMID
                    data:&i32
                  length:4];

  [self writeHeaderField:outputStream
                headerId:INNER_HEADER_RANDOMSTREAMKEY
                    data:protectedStreamKey.bytes
                  length:protectedStreamKey.length];

  for (NSData *bdata in tree.headerBinaries) {
    [self writeHeaderField:outputStream
                  headerId:INNER_HEADER_BINARY
                      data:bdata.bytes
                    length:bdata.length];
  }
...
- (void)newFile:(NSString *)fileName
  ...
  if (dbVer == DBVersionKdbx4) {
    tree.forcedVersion = KDBX40_VERSION;
    tree.kdfParams =
        [KdbPassword getDefaultKDFParameters:[KdbUUID getArgon2UUID]];
    tree.encryptionAlgorithm = [KdbUUID getChaCha20UUID];
  } else {
    tree.forcedVersion = KDBX31_VERSION;
    tree.kdfParams =
        [KdbPassword getDefaultKDFParameters:[KdbUUID getAES_KDFUUID]];
...
- (uint32_t)getMinDatabaseVersion:(Kdb4Tree *)tree {
  if (tree.forcedVersion != 0)
    return tree.forcedVersion;
...
- (BOOL)groupsHaveCustomData:(Kdb4Group *)group {
  if ([group.customData count] > 0)
    return YES;

  for (Kdb4Group *g in group.groups) {
    if ([self groupsHaveCustomData:g])
      return YES;
  }
