...
- (NSUInteger)write:(const void *)bytes length:(NSUInteger)bytesLength {
  ...
  [self ensureBufferCapacity:CCCryptorGetOutputLength(cryptorRef, bytesLength,
                                                      NO)];

  // Encrypt the data
  size_t n = 0;
  CCCryptorStatus cs = CCCryptorUpdate(cryptorRef, bytes, bytesLength, buffer,
                                       bufferCapacity, &n);
  if (cs != kCCSuccess) {
    @throw [NSException exceptionWithName:@"EncryptError"
